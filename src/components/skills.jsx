  ...
  useEffect(() => {
    if (animate) {
      ...
      function animateMeter(now) {
        ...
        const next = Math.min(
          percentage,
          Math.round((elapsed / duration) * percentage)
        );
        setProgress(next);
    ...
    <div className="circular-meter">
      <svg width="50" height="50">
        ...
        <circle
          ...
          style={{
            transition: animate
              ? "stroke-dashoffset 0.8s cubic-bezier(.4,2,.6,1)"
              : "none",
          }}
        />
        {showLabel && (
      ...
      <div className="skills-filter">
        ...
        <button
          className={`filter-btn ${
            activeFilter === "Tech Stack" ? "active" : ""
          }`}
        ...
        {getSkills().map((skill) => (
          ...
          >
            <img
              src={skill.iconUrl}
              alt={`${skill.name} logo`}
              className={`skill-icon${hovered === skill.name ? " zoomed" : ""}`}
            />
            ...
            <div className="skill-meter-horizontal">
              <CircularMeter
                ...
                animate={hovered === skill.name}
                showLabel={hovered === skill.name}
              />
            </div>
          </div>
        ))}
